...
export const createFriendAccount = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Existing account (e.g. signed in with Google earlier) still needs the friend role.
    if (created.error) {
      ...
      const id = list?.[0]?.id;
      if (id) {
        await supabaseAdmin.from("user_roles").upsert({ user_id: id, role: "friend" }, { onConflict: "user_id,role" });
      }
    }
    return { ok: true };
  });
