...
function withApiKey(key: string): typeof fetch {
  // New Supabase keys are opaque strings, so the bearer token must stay the
  // caller's own access token while `apikey` carries the project key.
  return (input, init) => {
...
/** Forwards the verified OAuth token so RLS runs as the signed-in reader. */
export function supabaseForUser(ctx: ToolContext) {
  ...
  return createClient(supabaseProjectUrl(), key, {
    global: { headers: { Authorization: `Bearer ${token}` }, fetch: withApiKey(key) },
